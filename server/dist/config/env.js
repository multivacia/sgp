...
function parsePortLabel(raw, label) {
    const s = raw.trim();
...
function parseOptionalPgPoolMax(v) {
...
function parseOptionalBooleanFlag(v) {
    if (v === undefined || v.trim() === '')
        return undefined;
    const l = v.trim().toLowerCase();
    if (['1', 'true', 'yes', 'on'].includes(l))
        return true;
    if (['0', 'false', 'no', 'off'].includes(l))
        return false;
    throw new Error('REQUIRE_DB_ON_STARTUP deve ser um de: 1, 0, true, false, yes, no, on, off');
}
/**
 ...
 */
export function resolvePgPoolConfig(processEnv) {
    ...
    if (passwordRaw === undefined || passwordRaw === '') {
        ...
        throw new Error('PGPASSWORD não pode ser vazio ou só espaços (Modo B). Use DATABASE_URL (Modo A) ou defina uma senha válida.');
    }
    if (missing.length > 0) {
        throw new Error([
 ...
 */
export function hasDatabaseConnectionInEnv(processEnv) {
    if (processEnv.DATABASE_URL?.trim())
        return true;
    return (!!processEnv.PGHOST?.trim() &&
...
function parseHealthInfraToken(raw) {
    if (raw === undefined)
        return undefined;
    const t = raw.trim();
    if (t === '')
        return undefined;
    if (t.length < 32) {
        throw new Error('HEALTH_INFRA_TOKEN, quando definido, deve ter pelo menos 32 caracteres.');
    }
    return t;
}
export function loadEnv() {
    ...
    const raw = {
        ...
        LOGIN_MAX_FAILED_ATTEMPTS: process.env.LOGIN_MAX_FAILED_ATTEMPTS?.trim() || '5',
        LOGIN_LOCKOUT_MINUTES: process.env.LOGIN_LOCKOUT_MINUTES?.trim() || '15',
    };
    ...
    try {
        ...
        throw new Error(['Falha ao resolver conexão com o PostgreSQL (server/.env).', msg].join('\n'));
    }
    let pgPoolMax;
    let requireDbOnStartup;
    try {
        ...
        const msg = e instanceof Error ? e.message : String(e);
        throw new Error(['Variáveis opcionais de banco inválidas (server/.env).', msg].join('\n'));
    }
    let healthInfraToken;
    try {
        healthInfraToken = parseHealthInfraToken(process.env.HEALTH_INFRA_TOKEN);
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        throw new Error(['HEALTH_INFRA_TOKEN inválido (server/.env).', msg].join('\n'));
    }
    /**
     ...
     */
    const forceLocalDocumentAdapter = process.env.SGP_TEST_LOCAL_DOCUMENT_ADAPTER === '1';
    ...
    if (rawRemoteReq !== undefined && rawRemoteReq !== '') {
        ...
        if (['1', 'true', 'yes', 'on'].includes(l))
            argosRemoteRequired = true;
        else if (['0', 'false', 'no', 'off'].includes(l))
            argosRemoteRequired = false;
        else {
            throw new Error('ARGOS_REMOTE_REQUIRED deve ser um de: 1, 0, true, false, yes, no, on, off.');
        }
    }
    ...
    if (rawStub !== undefined && rawStub !== '') {
        ...
        if (['1', 'true', 'yes', 'on'].includes(l))
            argosUseMinimalStub = true;
        else if (['0', 'false', 'no', 'off'].includes(l))
            argosUseMinimalStub = false;
        else {
            throw new Error('ARGOS_USE_MINIMAL_STUB deve ser um de: 1, 0, true, false, yes, no, on, off.');
        }
    }
    ...
    const argosCallerEnvironment = process.env.ARGOS_CALLER_ENVIRONMENT?.trim() || undefined;
    let argosPolicyMode = 'balanced';
    ...
    if (rawPolicy !== undefined && rawPolicy !== '') {
        if (rawPolicy === 'economy' || rawPolicy === 'balanced' || rawPolicy === 'quality') {
            ...
            throw new Error('ARGOS_POLICY_MODE deve ser um de: economy, balanced, quality.');
        }
    }
    ...
    if (rawTimeout !== undefined && rawTimeout !== '') {
        ...
        if (Number.isNaN(n) || n < 1000 || n > 3_600_000) {
            throw new Error('ARGOS_INGEST_TIMEOUT_MS deve ser um inteiro entre 1000 e 3600000 (ms).');
        }
    ...
    if (rawMax !== undefined && rawMax !== '') {
        ...
        if (Number.isNaN(n) || n < 1024 || n > 200 * 1024 * 1024) {
            throw new Error('DOCUMENT_DRAFT_MAX_FILE_BYTES deve ser um inteiro entre 1024 e 209715200.');
        }
    ...
    const supportEmailSubjectPrefix = process.env.SUPPORT_EMAIL_SUBJECT_PREFIX?.trim() || '[SGP]';
    ...
    const smtpPass = process.env.SMTP_PASS !== undefined ? process.env.SMTP_PASS : undefined;
    const smtpRequireTls = parseOptionalBooleanFlag(process.env.SMTP_REQUIRE_TLS) ?? smtpPort === 587;
    const v = parsed.data;
    const databaseUrl = 'connectionString' in pgPoolConfig ? pgPoolConfig.connectionString : undefined;
    cached = {
        nodeEnv: v.NODE_ENV,
        port: v.PORT,
        pgPoolConfig,
        ...
        supportRoutingDefaultWhatsapp: process.env.SUPPORT_ROUTING_DEFAULT_WHATSAPP?.trim() || undefined,
