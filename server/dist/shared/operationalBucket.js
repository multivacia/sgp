...
export function parseFlexibleDeadlineToDate(value) {
...
function isEsteiraOverdueVersusToday(row, now = new Date()) {
...
function mapDbStatusToUi(operationalStatus) {
...
export function getOperationalBucketForConveyor(operationalStatus, estimatedDeadline, now = new Date()) {
    const status = mapDbStatusToUi(operationalStatus);
    const row = { status, estimatedDeadline };
    if (status === 'concluida')
        return 'concluidas';
    if (isEsteiraOverdueVersusToday(row, now))
        return 'em_atraso';
...
export function operationalBucketSortRank(b) {
    const order = {
        em_atraso: 0,
        em_revisao: 1,
        em_andamento: 2,
...
//# sourceMappingURL=operationalBucket.js.map