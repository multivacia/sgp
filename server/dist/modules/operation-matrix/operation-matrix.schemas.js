...
export const listMatrixItemsQuerySchema = z
    ...
    .transform((data) => {
    const search = data.search?.trim() === '' || data.search === undefined
        ? undefined
        : data.search.trim();
    ...
    if (!isActivity) {
        if (data.plannedMinutes !== undefined) {
            ctx.addIssue({
                ...
                path: ['plannedMinutes'],
                message: 'planned_minutes só é aceito para nós do tipo ACTIVITY.',
            });
        }
        if (data.defaultResponsibleId !== undefined) {
            ctx.addIssue({
                ...
                message: 'default_responsible_id só é aceito para nós do tipo ACTIVITY.',
            });
        }
...
//# sourceMappingURL=operation-matrix.schemas.js.map