...
export function buildSanitizedMetadata(eventType, raw) {
    ...
    for (const key of allow) {
        ...
        if (key === 'changed_fields') {
            if (!Array.isArray(v))
                throw new Error('changed_fields deve ser array');
        ...
        if (key === 'via') {
            if (v !== 'endpoint' && v !== 'patch')
                throw new Error('via inválido');
            out[key] = v;
            continue;
        }
        ...
        if (key === 'cleared_lockout') {
            if (typeof v !== 'boolean')
                throw new Error('cleared_lockout deve ser boolean');
            out[key] = v;
            continue;
        }
        if (key === 'initial_role_id' || key === 'new_collaborator_id') {
            if (!isUuidString(v))
                throw new Error(`${key} deve ser UUID`);
            out[key] = v;
            continue;
        }
        if (key === 'previous_collaborator_id') {
            ...
            if (!isUuidString(v))
                throw new Error('previous_collaborator_id inválido');
            out[key] = v;
            continue;
        }
        if (key === 'role_id') {
            if (!isUuidString(v))
                throw new Error('role_id deve ser UUID');
            out[key] = v;
            continue;
        }
        if (key === 'role_code') {
            if (typeof v !== 'string' || !v.trim())
                throw new Error('role_code inválido');
            out[key] = v;
            continue;
        }
        if (key === 'added_permission_codes' || key === 'removed_permission_codes') {
            ...
            if (arr.some((s) => !s.trim()))
                throw new Error(`${key} contém entrada inválida`);
            out[key] = arr;
            continue;
        }
        if (key === 'conveyor_id' ||
        ...
        if (key === 'reason') {
            if (typeof v !== 'string')
                throw new Error('reason deve ser texto');
            const t = v.trim();
            if (!t.length)
                throw new Error('reason não pode ser vazio');
            if (t.length > 4000)
                throw new Error('reason excede tamanho máximo');
            out[key] = t;
            continue;
        }
    }
...
//# sourceMappingURL=admin-audit.metadata.js.map