...
import { listStepHierarchyForConveyor, sumRealizedMinutesByStepForConveyor, } from './conveyorNodeWorkload.repository.js';
const NOTES = 'Pendência de tempo compara o previsto estrutural do STEP com minutos apontados acumulados na base. Não identifica causa raiz. O indicador de pressão de atraso refere-se à esteira (bucket operacional), não ao STEP.';
function plannedNum(p) {
    if (p == null || Number.isNaN(p))
        return 0;
    return Math.max(0, p);
}
function pendingMinutes(planned, realized) {
...
export async function serviceGetConveyorNodeWorkload(pool, conveyorId) {
    ...
    const areas = [...areaMap.values()].sort((a, b) => b.pendingMinutesSum - a.pendingMinutesSum);
    const bucket = getOperationalBucketForConveyor(conv.operational_status, conv.estimated_deadline);
    return {
...
//# sourceMappingURL=conveyorNodeWorkload.service.js.map