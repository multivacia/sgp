...
import { countActiveTimeEntriesByConveyor, deleteConveyorAssigneesAndNodes, findConveyorById, insertConveyor, insertConveyorNode, listConveyorNodesByConveyorId, listConveyors, newNodeId, updateConveyorDados, updateConveyorOperationalStatus, updateConveyorStructureMeta, } from './conveyors.repository.js';
import { insertConveyorNodeAssignee, newAssignmentId, } from './conveyorAssignments.repository.js';
...
function assertUniqueOrderIndices(items, label) {
    const seen = new Set();
    for (const it of items) {
        if (seen.has(it.orderIndex)) {
            throw new AppError(`${label}: orderIndex duplicado.`, 422, ErrorCodes.VALIDATION_ERROR);
        }
        seen.add(it.orderIndex);
    }
}
function computeTotalsForOptions(options) {
    let totalAreas = 0;
    let totalSteps = 0;
...
function parseConveyorMetadataJson(m) {
...
function mapDetailRowToApi(row, structure) {
...
function resolveCompletedAtMode(current, next) {
    if (next === 'CONCLUIDA')
        return 'now';
...
export function buildConveyorStructureFromNodes(rows) {
    const options = rows
...
export async function serviceGetConveyorById(pool, id) {
...
export async function servicePatchConveyorStatus(pool, conveyorId, nextStatus) {
    const row = await findConveyorById(pool, conveyorId);
    if (!row)
        return null;
    if (row.operational_status === nextStatus) {
        throw new AppError('Não é permitido alterar para o mesmo status.', 422, ErrorCodes.INVALID_STATUS_TRANSITION);
    }
    if (!isTransitionAllowed(row.operational_status, nextStatus)) {
        throw new AppError(`Não é permitido mudar de ${row.operational_status} para ${nextStatus}.`, 422, ErrorCodes.INVALID_STATUS_TRANSITION);
    }
    const mode = resolveCompletedAtMode(row.operational_status, nextStatus);
    const updated = await updateConveyorOperationalStatus(pool, conveyorId, nextStatus, mode);
    if (!updated)
        return null;
...
export async function serviceListConveyors(pool, filters) {
    const rows = await listConveyors(pool, filters);
    return rows.map(mapListRowToApi);
}
function revalidateStructureOptions(options) {
    const sortedOptions = [...options].sort((a, b) => a.orderIndex - b.orderIndex);
...
function revalidateStructure(body) {
    revalidateStructureOptions(body.options);
}
function collectAssigneeTargetsFromOptions(options) {
        ...
        for (const ar of op.areas) {
            for (const st of ar.steps) {
                for (const a of st.assignees ?? []) {
                    ...
                    if (a.collaboratorId)
                        collaboratorIds.add(a.collaboratorId);
                }
            }
        }
    }
...
function collectAssigneeTargets(body) {
    return collectAssigneeTargetsFromOptions(body.options);
}
async function materializeConveyorOptions(client, conveyorId, options) {
    const sortedOptions = [...options].sort((a, b) => a.orderIndex - b.orderIndex);
    for (const op of sortedOptions) {
...
export async function serviceCreateConveyor(pool, body) {
    ...
    if (dados.colaboradorId) {
        ...
        if (!ok) {
            throw new AppError('Colaborador (responsável) não encontrado.', 422, ErrorCodes.VALIDATION_ERROR);
        }
    }
    ...
    for (const cid of assigneeTargets.collaboratorIds) {
        ...
        if (!ok) {
            throw new AppError('Colaborador de alocação inexistente, inativo ou indisponível.', 422, ErrorCodes.VALIDATION_ERROR);
        }
    }
    for (const tid of assigneeTargets.teamIds) {
        const t = await findTeamById(pool, tid);
        if (!t || !t.is_active || t.deleted_at) {
            throw new AppError('Time de alocação inexistente ou inativo.', 422, ErrorCodes.VALIDATION_ERROR);
        }
    }
...
function mergeConveyorMetadata(current, patch) {
    const cur = parseConveyorMetadataJson(current);
    return {
        colaboradorId: patch.colaboradorId !== undefined ? patch.colaboradorId : cur.colaboradorId,
        matrixRootItemId: patch.matrixRootItemId !== undefined
            ? patch.matrixRootItemId
            : cur.matrixRootItemId,
    };
}
export async function servicePatchConveyorDados(pool, conveyorId, body) {
...
export async function serviceReplaceConveyorStructure(pool, conveyorId, body) {
    const existing = await findConveyorById(pool, conveyorId);
    if (!existing)
        return null;
    if (existing.operational_status !== 'NO_BACKLOG' &&
        existing.operational_status !== 'EM_REVISAO') {
        throw new AppError('Substituição de estrutura só é permitida quando a esteira está no backlog ou em revisão.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    const nEntries = await countActiveTimeEntriesByConveyor(pool, conveyorId);
    if (nEntries > 0) {
        throw new AppError('Não é possível substituir a estrutura: existem apontamentos registados nesta esteira.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    revalidateStructureOptions(body.options);
    ...
    const metaNext = mergeConveyorMetadata(existing.metadata_json, {
        matrixRootItemId: body.matrixRootItemId === undefined ? undefined : body.matrixRootItemId,
    });
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
