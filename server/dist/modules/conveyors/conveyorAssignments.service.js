...
import { findAppUserEmailById, findCollaboratorIdByAppUserId, } from '../auth/auth.repository.js';
import { insertAdminAuditEvent } from '../admin-audit/admin-audit.repository.js';
import { AppError } from '../../shared/errors/AppError.js';
import { ErrorCodes } from '../../shared/errors/errorCodes.js';
import { assigneeRowToCreated, assigneeListRowToDto, timeEntryRowToCreated, timeEntryListRowToDto, } from './conveyorAssignments.dto.js';
import { findAssigneeIdForStepAndCollaborator, findConveyorNodeById, findConveyorNodeAssigneeById, findConveyorTimeEntryById, insertConveyorNodeAssignee, insertConveyorTimeEntry, listConveyorNodeAssigneesByStep, listConveyorTimeEntriesByStep, newAssignmentId, softDeleteConveyorNodeAssignee, softDeleteConveyorTimeEntry, } from './conveyorAssignments.repository.js';
...
function isPgCheckOrRaise(err) {
    return err instanceof DatabaseError && (err.code === '23514' || err.code === 'P0001');
}
export async function collaboratorActiveForOperations(pool, collaboratorId) {
 ...
 */
export async function assertNodeIsStepForConveyor(pool, conveyorId, conveyorNodeId) {
    ...
    if (node.conveyor_id !== conveyorId) {
        throw new AppError('conveyor_id incompatível com o nó.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    if (node.node_type !== 'STEP') {
        throw new AppError('Operação permitida apenas em atividades (STEP).', 422, ErrorCodes.VALIDATION_ERROR);
    }
}
export async function serviceCreateConveyorNodeAssignee(pool, input) {
    const type = input.type ?? 'COLLABORATOR';
    if (type === 'COLLABORATOR') {
        if (!input.collaboratorId) {
            throw new AppError('collaboratorId é obrigatório para assignee COLLABORATOR.', 422, ErrorCodes.VALIDATION_ERROR);
        }
        const ok = await collaboratorActiveForOperations(pool, input.collaboratorId);
        if (!ok) {
            throw new AppError('Colaborador inexistente, inativo ou indisponível.', 422, ErrorCodes.VALIDATION_ERROR);
        }
    }
    else {
        if (!input.teamId) {
            throw new AppError('teamId é obrigatório para assignee TEAM.', 422, ErrorCodes.VALIDATION_ERROR);
        }
        if (input.isPrimary) {
            throw new AppError('Assignee TEAM não pode ser principal.', 422, ErrorCodes.VALIDATION_ERROR);
        }
        const t = await findTeamById(pool, input.teamId);
        if (!t || !t.is_active || t.deleted_at) {
            throw new AppError('Time inexistente ou inativo.', 422, ErrorCodes.VALIDATION_ERROR);
        }
    }
    ...
    try {
        ...
        if (isPgUniqueViolation(err)) {
            throw new AppError(type === 'TEAM'
                ? 'Time já alocado nesta atividade.'
                : 'Colaborador já alocado nesta atividade, ou já existe um responsável principal.', 409, ErrorCodes.CONFLICT);
        }
        if (isPgCheckOrRaise(err)) {
            throw new AppError(err instanceof Error ? err.message : 'Regra de integridade violada.', 422, ErrorCodes.VALIDATION_ERROR);
        }
        throw err;
    }
...
export async function serviceListConveyorNodeAssignees(pool, conveyorId, conveyorNodeId) {
...
export async function serviceDeleteConveyorNodeAssignee(pool, conveyorId, conveyorNodeId, assigneeId) {
    await assertNodeIsStepForConveyor(pool, conveyorId, conveyorNodeId);
 ...
 */
export async function serviceCreateConveyorTimeEntryForAppUser(pool, input) {
    await assertNodeIsStepForConveyor(pool, input.conveyorId, input.conveyorNodeId);
    const collaboratorId = await findCollaboratorIdByAppUserId(pool, input.appUserId);
    if (!collaboratorId) {
        throw new AppError('Conta sem colaborador operacional vinculado. Contacte o administrador para associar o seu utilizador a um colaborador.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    const assigneeId = await findAssigneeIdForStepAndCollaborator(pool, input.conveyorId, input.conveyorNodeId, collaboratorId);
    if (!assigneeId) {
        throw new AppError('Não está alocado nesta atividade. Não é possível apontar.', 403, ErrorCodes.FORBIDDEN);
    }
...
export async function serviceCreateConveyorTimeEntry(pool, input) {
    if (input.minutes <= 0) {
        throw new AppError('minutes deve ser maior que zero.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    const ok = await collaboratorActiveForOperations(pool, input.collaboratorId);
    if (!ok) {
        throw new AppError('Colaborador inexistente, inativo ou indisponível.', 422, ErrorCodes.VALIDATION_ERROR);
 ...
 */
export async function serviceCreateConveyorTimeEntryOnBehalf(pool, input) {
    const reason = input.reason.trim();
    if (!reason.length) {
        throw new AppError('Indique o motivo do apontamento em nome de outro colaborador.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    if (reason.length > DELEGATION_REASON_MAX) {
        throw new AppError('Motivo excede o tamanho máximo permitido.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    if (input.minutes <= 0) {
        throw new AppError('minutes deve ser maior que zero.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    const targetOk = await collaboratorActiveForOperations(pool, input.targetCollaboratorId);
    if (!targetOk) {
        throw new AppError('Colaborador inexistente, inativo ou indisponível.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    await assertNodeIsStepForConveyor(pool, input.conveyorId, input.conveyorNodeId);
    const assigneeId = await findAssigneeIdForStepAndCollaborator(pool, input.conveyorId, input.conveyorNodeId, input.targetCollaboratorId);
    if (!assigneeId) {
        throw new AppError('O colaborador indicado não está alocado nesta atividade.', 422, ErrorCodes.VALIDATION_ERROR);
    }
    const metadataJson = {
...
export async function serviceListConveyorTimeEntries(pool, conveyorId, conveyorNodeId) {
    await assertNodeIsStepForConveyor(pool, conveyorId, conveyorNodeId);
    const rows = await listConveyorTimeEntriesByStep(pool, conveyorId, conveyorNodeId);
    return rows.map(timeEntryListRowToDto);
}
export async function serviceDeleteConveyorTimeEntry(pool, conveyorId, conveyorNodeId, timeEntryId) {
    await assertNodeIsStepForConveyor(pool, conveyorId, conveyorNodeId);
...
export async function serviceDeleteConveyorTimeEntryAsAppUser(pool, input) {
    ...
    if (!collaboratorId || entry.collaborator_id !== collaboratorId) {
        throw new AppError('Não foi possível remover este apontamento.', 403, ErrorCodes.FORBIDDEN);
    }
    return serviceDeleteConveyorTimeEntry(pool, input.conveyorId, input.conveyorNodeId, input.timeEntryId);
}
//# sourceMappingURL=conveyorAssignments.service.js.map