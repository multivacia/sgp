...
const RE_CURRENCY_OR_UNIT_ALONE = /^(r\$|real\.?|reais\.?|km|km\/h|cv|hp|v\d{1,2}\.?\d*)$/i;
 ...
 */
export function isPlausibleClientName(s) {
    const t = s.trim();
    if (t.length < 2)
        return false;
    if (!hasLetter(t))
        return false;
    if (RE_JUNK_TOKEN.test(t))
        return false;
    if (t.length <= 6 && RE_CURRENCY_OR_UNIT_ALONE.test(t))
        return false;
    if (/^cliente\s*:?\s*$/i.test(t))
        return false;
    return true;
}
/**
 ...
 */
export function isPlausibleVehicleDescription(s) {
    const t = s.trim();
    if (t.length < 3)
        return false;
    if (!hasLetter(t))
    ...
    if (/^r\$\s*[\d.,]+\s*$/i.test(t))
        return false;
    if (RE_CURRENCY_OR_UNIT_ALONE.test(t))
        return false;
    /** Linha só com marca de moeda ou “código” curto demais sem letras suficientes */
    if (t.length <= 8 && /^[\d.,\sR$r€£]+$/i.test(t))
        return false;
    return true;
}
const RE_MODEL_STRONG = /\b(?:modelo|vers[aã]o|vers\.)\s*[:.\-]?\s*([^\n]{2,100})/i;
/**
 * Só preenche modelVersion quando há rótulo explícito (modelo/versão) e texto plausível.
 */
export function extractModelVersionStrongSignal(text) {
    ...
    if (!hasLetter(cut))
        return undefined;
    if (RE_JUNK_TOKEN.test(cut))
        return undefined;
    if (cut.length > 120)
        return cut.slice(0, 120).trim();
    return cut;
}
//# sourceMappingURL=documentFieldPlausibilityBr.js.map