...
import { findRoleById, getPermissionCodesForRole, listPermissionsCatalog, listRoles, replaceRolePermissions, resolvePermissionIdsByCodes, } from './rbac.repository.js';
...
export async function serviceListRbacRoles(pool) {
    return listRoles(pool);
}
export async function serviceListRbacPermissionsCatalog(pool) {
    return listPermissionsCatalog(pool);
}
export async function serviceGetRolePermissionCodes(pool, roleId) {
...
function assertAdminSafeguards(roleCode, nextCodes) {
    if (roleCode !== ADMIN_ROLE_CODE)
        return;
    const missing = ADMIN_REQUIRED_PERMISSION_CODES.filter((c) => !nextCodes.has(c));
    if (missing.length > 0) {
        throw new AppError(`O papel Administrador deve manter as permissões: ${missing.join(', ')}.`, 400, ErrorCodes.VALIDATION_INVALID_INPUT);
    }
}
export async function servicePutRolePermissions(pool, actorUserId, roleId, body) {
    const role = await findRoleById(pool, roleId);
    if (!role) {
        throw new AppError('Papel não encontrado.', 404, ErrorCodes.OPERATIONAL_RESOURCE_NOT_FOUND);
    }
    if (role.code === COLABORADOR_ROLE_CODE) {
        throw new AppError('O papel Colaborador não pode ser editado nesta versão (sem permissões RBAC explícitas).', 403, ErrorCodes.RBAC_DENIED);
    }
    ...
    if (resolved.size !== uniqueCodes.length) {
        ...
        throw new AppError(`Código(s) de permissão desconhecido(s): ${missing.join(', ')}`, 400, ErrorCodes.VALIDATION_INVALID_INPUT);
    }
