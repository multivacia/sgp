 ...
 */
export async function countActiveUsersWithoutCollaboratorLink(db) {
...
export async function countAdminUsers(db, filters) {
...
export async function listAdminUsers(db, filters) {
    const { sql, values } = buildListWhere(filters);
...
export async function findAdminUserById(db, id) {
    const r = await db.query(`${listSelect} WHERE u.id = $1::uuid`, [id]);
    const row = r.rows[0];
    return row ? rowToItem(row) : null;
}
export async function insertAppUser(db, input) {
    const r = await db.query(`
...
export async function patchAppUser(db, id, patch) {
    const sets = ['updated_at = now()', `updated_by = $1::uuid`];
    const vals = [patch.updated_by];
    let n = 2;
...
export async function setUserActive(db, id, isActive, updatedBy) {
    const r = await db.query(`
    UPDATE app_users
...
export async function softDeleteUser(db, id, updatedBy) {
    const r = await db.query(`
    UPDATE app_users
    SET
      deleted_at = now(),
...
export async function restoreUser(db, id, updatedBy) {
    const r = await db.query(`
    UPDATE app_users
    SET
      deleted_at = NULL,
...
export async function setForcePasswordChange(db, id, updatedBy) {
    const r = await db.query(`
    UPDATE app_users
...
export async function resetAdminUserPasswordHash(db, id, passwordHash, updatedBy) {
    const r = await db.query(`
    UPDATE app_users
    SET
      password_hash = $2,
 ...
 */
export async function setAppUserPasswordExplicit(db, id, passwordHash, updatedBy) {
    const r = await db.query(`
    UPDATE app_users
    SET
      password_hash = $2,
...
export async function assertCollaboratorEligibleForLink(db, collaboratorId, excludeUserId) {
    const r = await db.query(`
    SELECT EXISTS (
      SELECT 1
...
export async function listEligibleCollaboratorsForLink(db, excludeUserId) {
    const r = await db.query(`
    SELECT
      c.id::text,
      c.full_name,
...
export async function roleExists(db, roleId) {
    const r = await db.query(`SELECT EXISTS (SELECT 1 FROM app_roles WHERE id = $1::uuid) AS ok`, [roleId]);
