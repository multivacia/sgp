...
import { findCollaboratorBrief, listTimeEntriesForCollaboratorInPeriod, sumRealizedMinutesInPeriodForCollaborator, sumRealizedMinutesTotalForCollaborator, } from './operational-journey.repository.js';
const COBERTURA_FORMULA = 'realizado_minutos_acumulados_nos_steps_alocados / soma_planned_minutes_nos_steps_alocados (escopo fechado; null se previsto ≤ 0)';
...
function parseIsoDate(value, field) {
    ...
    if (Number.isNaN(d.getTime())) {
        throw new AppError(`Parâmetro ${field} inválido: use data/hora ISO 8601.`, 400, ErrorCodes.VALIDATION_ERROR);
    }
    return d;
}
function resolveJourneyPeriod(q) {
    const preset = q.periodPreset;
    if (preset === 'custom') {
        const from = parseIsoDate(q.from, 'from');
        const to = parseIsoDate(q.to, 'to');
        if (from.getTime() > to.getTime()) {
            throw new AppError('Intervalo inválido: `from` deve ser anterior ou igual a `to`.', 400, ErrorCodes.VALIDATION_ERROR);
        }
...
export async function serviceGetOperationalJourney(pool, args) {
...
//# sourceMappingURL=operational-journey.service.js.map