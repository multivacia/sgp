...
import { listMySupportTicketsQuerySchema, type ListMySupportTicketsQuery } from './support.schemas.js'
import type {
...
function toDispatchItems(plan: ReturnType<typeof resolveRoutingPlan>): DispatchableNotification[] {
  return plan.items.flatMap((item) => item.destinations.map((destination) => ({ channel: item.channel, destination })))
}

function buildSummary(results: DispatchResult[]): NotificationSummary {
  const grouped = {
    EMAIL: results.filter((r) => r.channel === 'EMAIL'),
    WHATSAPP: results.filter((r) => r.channel === 'WHATSAPP'),
  }
  const summarize = (items: DispatchResult[]): 'SENT' | 'FAILED' | 'SKIPPED' | 'PENDING' => {
...
export async function createSupportTicket(
  ...
  await repository.insertNotificationAttempts(
    pool,
    ticket.id,
...
export async function getMySupportTicketById(
  pool: pg.Pool,
  env: Env,
  userId: string,
  ticketId: string,
  ...
  if (!ticket) throw new AppError('Chamado não encontrado.', 404, ErrorCodes.NOT_FOUND)
  return ticket
}
...
export async function listMySupportTickets(
  ...
  env: Env,
  userId: string,
  filters?: ListMySupportTicketsQuery,
) {
  if (!env.supportTicketsEnabled) {
    throw new AppError('Módulo de suporte está desativado.', 404, ErrorCodes.NOT_FOUND)
  }
