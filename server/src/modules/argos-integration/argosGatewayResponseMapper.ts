...
type QualityConfidence = 'high' | 'medium'
type QualityAmbiguity = 'low' | 'medium'

function isRecord(v: unknown): v is Record<string, unknown> {
  return v !== null && typeof v === 'object' && !Array.isArray(v)
}

function confTo01(c: 'low' | 'medium' | 'high'): number {
  switch (c) {
    case 'high':
      return 0.88
    case 'medium':
...
function overallConfidence(
  ...
  if (c === 'high' && a === 'low') return 0.92
  if (c === 'high') return 0.78
  if (c === 'medium' && a === 'low') return 0.62
  return 0.48
}
 ...
 */
export function mapArgosGatewayDocumentIngestToSgp(
  raw: unknown,
  ...
  const document: ArgosDocumentIngestResult['document'] = {}
  if (isRecord(artifact)) {
    if (typeof artifact.fileName === 'string') document.fileName = artifact.fileName
    if (typeof artifact.mimeType === 'string') document.mimeType = artifact.mimeType
  }

  const extractedFacts: ArgosDocumentIngestResult['extractedFacts'] = []
  if (isRecord(draftBlock) && isRecord(draftBlock.result)) {
    const consolidated = (draftBlock.result as { consolidated?: unknown }).consolidated
  ...
  const warnings: ArgosDocumentIngestResult['warnings'] = []
  ...
  const qc: QualityConfidence =
    qcRaw === 'high' || qcRaw === 'medium' ? qcRaw : 'medium'
  const qa: QualityAmbiguity =
    qaRaw === 'low' || qaRaw === 'medium' ? qaRaw : 'medium'
  const confidence = {
    overall: overallConfidence(qc, qa),
  }

  const novaResult =
    isRecord(novaBlock) && isRecord(novaBlock.result) ? novaBlock.result : null
  ...
  const specialist =
    isRecord(novaTrace) && typeof novaTrace.specialistKey === 'string'
      ? novaTrace.specialistKey
      : typeof trace.specialistKey === 'string'
        ? trace.specialistKey
        : 'argos_document_gateway'
  const strategy =
    isRecord(novaTrace) && typeof novaTrace.strategyUsed === 'string'
      ? novaTrace.strategyUsed
      : typeof trace.strategyUsed === 'string'
        ? trace.strategyUsed
        : 'pdf_ingest_v1'

  const sgpStatus: ArgosDocumentIngestResult['status'] =
    statusRaw === 'SUCCESS_WITH_WARNINGS' ? 'partial' : 'completed'

  const result: ArgosDocumentIngestResult = {
    requestId,
    correlationId,
    status: sgpStatus,
    specialist,
  ...
  if (!checked.success) {
    throw new Error(
      'Mapeamento interno: resultado não passou na validação do contrato SGP+.',
    )
  }
  return checked.data
}

function buildConveyorDraftV1FromNova(
  novaResult: Record<string, unknown> | null,
  extractedFacts: ArgosDocumentIngestResult['extractedFacts'],
): ArgosDocumentIngestResult['draft'] {
  if (!novaResult) {
  ...
  const suggestedDados: NonNullable<
    ArgosDocumentIngestResult['draft']
  >['suggestedDados'] = {}

  if (structural && isRecord(structural.documentPrincipal)) {
    const dp = structural.documentPrincipal as Record<string, unknown>
    const titleHint = typeof dp.titleHint === 'string' ? dp.titleHint.trim() : ''
    if (titleHint) suggestedDados.title = titleHint.slice(0, 500)
    const summaryLine =
      typeof dp.summaryLine === 'string' ? dp.summaryLine.trim() : ''
    ...
    if (Array.isArray(idCandidates)) {
      for (const c of idCandidates) {
        ...
        if (c.semanticRole === 'order_number') {
          const pv =
            typeof c.valuePreview === 'string' ? c.valuePreview.trim() : ''
          if (pv) {
            ;(suggestedDados as Record<string, string>).osNumber = pv.slice(
              0,
              200,
            )
          }
    ...
    const risks = businessView && Array.isArray(businessView.risksAndGaps)
      ? (businessView.risksAndGaps as string[])
    ...
    if (s) suggestedDados.notes = s.slice(0, 8000)
  }

  if (structural && Array.isArray(structural.mappedFields)) {
    for (const mf of structural.mappedFields) {
      if (!isRecord(mf)) continue
      const label =
        typeof mf.label === 'string' ? mf.label.toLowerCase() : ''
      const preview =
        typeof mf.valuePreview === 'string' ? mf.valuePreview.trim() : ''
      if (!preview) continue
  ...
  const format =
    typeof novaResult.format === 'string' ? novaResult.format : undefined

  const draft: NonNullable<ArgosDocumentIngestResult['draft']> = {
  ...
  if (!draft.suggestedDados.title?.trim() && businessView) {
    const summary =
      typeof businessView.summary === 'string'
        ? businessView.summary.trim().slice(0, 500)
        : ''
    if (summary) draft.suggestedDados.title = summary
  }

  if (
    !draft.suggestedDados.title?.trim() &&
...
function buildOptionsFromBusinessView(
  businessView: Record<string, unknown> | null,
): NonNullable<ArgosDocumentIngestResult['draft']>['options'] {
...
function minimalDraftFromFacts(
  extractedFacts: ArgosDocumentIngestResult['extractedFacts'],
): ArgosDocumentIngestResult['draft'] {
  const title =
    extractedFacts.length > 0 && typeof extractedFacts[0].value === 'string'
 ...
 */
export function parseArgosRemoteDocumentIngestPayload(
  json: unknown,
): ArgosDocumentIngestResult {
  const body = json
  if (!isRecord(body)) {
  ...
  throw new Error(
    'Resposta ARGOS não corresponde ao contrato documental (gateway nem legado SGP+).',
  )
}

function tryMapArgosGatewayPayload(
  payload: unknown,
): ArgosDocumentIngestResult | null {
  try {
