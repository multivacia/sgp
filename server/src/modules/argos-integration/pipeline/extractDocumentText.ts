...
export type ExtractDocumentTextResult = {
  ...
  extractWarnings: Array<{ code: string; message: string }>
}

function looksPdfMagic(buf: Buffer): boolean {
...
export async function extractDocumentText(params: {
  ...
  const warnings: ExtractDocumentTextResult['extractWarnings'] = []
  ...
  const asPdf =
    mime === 'application/pdf' || (!mime && looksPdfMagic(buffer)) || looksPdfMagic(buffer)
  ...
  if (asPdf) {
    try {
      ...
      warnings.push({
        ...
        message:
          'Não foi possível ler a estrutura do PDF; texto obtido por fallback pode ser incompleto.',
      })
