...
const RE_CNPJ =
  /\b\d{2}\.\d{3}\.\d{3}\s*\/\s*\d{4}-\d{2}\b|\b\d{14}\b/
const RE_CPF = /\b\d{3}\.\d{3}\.\d{3}-\d{2}\b/

/** Lexico leve de oficina / tapeçaria — evita matar serviços curtos operacionais. */
const RE_OPERATIONAL_LEXICON =
  /\b(porta[\s-]?malas|costura|costuras|espuma|lateral|laterais|bancos?|couro|revestimento|reformas?|reforma|trocas?|troca|refor[cç]o|aplica[cç][aã]o|sint[eé]ticos?|pe[cç]as?|tapetes?|carpetes?|limpezas?|polimento|aparente|costura|estofados?|laterais?|portas?|forros?|capas?)\b/i
 ...
 */
const RE_LIST_MARKER =
  /^\d{1,2}[\).\-]\s+|^\d{1,2}\s+(?=\p{L})|^[-•*]\s+/u
 ...
 */
export function normalizeDocumentLine(raw: string): string {
 ...
 */
export function splitGluedOrdinalPrefix(line: string): string {
...
const BLOCK_RULES: BlockRule[] = [
  ...
  {
    test: (s) =>
      /\b(LTDA\.?|EIRELI|E\.P\.P\.?|S\.A\.?|ME\.)\s*$/i.test(s) && s.length > 12,
    category: 'metadata',
    reason: 'company_suffix',
  },
  {
    test: (s) =>
      /^(ordem\s+de\s+servi[çc]o|ordem\s+de\s+servi[çc]o\s*[-–—]\s*os)\b/i.test(s),
    category: 'label',
  ...
  {
    test: (s) =>
      /documento\s+fict[ií]cio|simula[cç][aã]o|n[aã]o\s+correspondem\s+a|fins?\s+de\s+teste/i.test(
        s,
      ),
    category: 'disclaimer',
  ...
  {
    test: (s) =>
      /^observa[cç][oõ]es\s+(?:comerciais\s+e\s+técnicas|comerciais|técnicas)\s*:?\s*$/i.test(
        s,
      ),
    category: 'label',
  ...
  {
    test: (s) =>
      /^placa\s+[A-Z]{3}[\s-]?\d{4}\s*$/i.test(s) ||
  ...
  {
    test: (s) =>
      /^(servi[cç]o|pe[cç]a|item|descri[cç][aã]o|qtd|quantidade)\s*:?\s*$/i.test(s),
    category: 'label',
...
function applyHardBlocks(content: string): { category: DocumentLineCategory; reason: string } | null {
...
function computeOperationalScore(params: {
  content: string
  strippedFromRaw: string
  hadListMarker: boolean
  hasPriceToken: boolean
}): number {
  const { content, hadListMarker, hasPriceToken } = params
  const len = content.length
  let score = 0

  if (hadListMarker) score += 0.34
  if (hasPriceToken) score += 0.28

  if (/[a-záéíóúãõç]/i.test(content)) score += 0.12

  if (len >= 8 && len <= 160) score += 0.14
  else if (len >= 4 && len <= 7) score += 0.06
  else if (len > 160 && len <= MAX_LINE_LEN) score += 0.05

  if (RE_OPERATIONAL_LEXICON.test(content)) score += 0.26

  if (RE_HYPHENATED_NOUN.test(content)) score += 0.22

  /** Nome composto curto sem verbo explícito (ex.: "Espuma lateral"). */
  if (len >= 6 && len <= 48 && /^[\p{L}\p{M}\s\-–]+$/u.test(content)) {
    const words = content.split(/\s+/).filter(Boolean)
    if (words.length >= 1 && words.length <= 5) score += 0.12
  }
 ...
 */
export function classifyDocumentLine(rawLine: string): ClassifyDocumentLineResult {
  ...
  let category: DocumentLineCategory
 ...
 */
export function shouldPromoteToServiceLine(result: ClassifyDocumentLineResult): boolean {
