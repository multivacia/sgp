...
import type { InsertAdminAuditInput } from '../admin-audit/admin-audit.repository.js'
...
import {
  assertCollaboratorEligibleForLink,
  countActiveUsersWithoutCollaboratorLink,
  countAdminUsers,
  findAdminUserById,
  insertAppUser,
  listAdminUsers,
  listEligibleCollaboratorsForLink,
  patchAppUser,
  restoreUser,
  roleExists,
  resetAdminUserPasswordHash,
  setAppUserPasswordExplicit,
  setForcePasswordChange,
  setUserActive,
  softDeleteUser,
  type AdminUserListFilters,
  type InsertAppUserInput,
  type PatchAppUserInput,
...
export async function serviceCollaboratorLinkageSummary(
  pool: pg.Pool,
): Promise<{ unlinkedActiveUserCount: number }> {
  const unlinkedActiveUserCount =
    await countActiveUsersWithoutCollaboratorLink(pool)
...
function handlePg(e: unknown): never {
  if (isPgError(e)) {
    ...
    if (e.code === '23503') {
      throw new AppError(
        'Papel ou colaborador informado não existe.',
        422,
...
function emptyUrl(s: string | null | undefined): string | null {
...
function projectAfterPatch(
  existing: AdminUserListItem,
...
function buildPatchAuditPlan(
  actorUserId: string,
  targetUserId: string,
  existing: AdminUserListItem,
  projected: AdminUserListItem,
  body: PatchAdminUserBody,
): InsertAdminAuditInput[] {
  const out: InsertAdminAuditInput[] = []
  ...
  if (existing.email !== projected.email) changed.push('email')
  if (existing.roleId !== projected.roleId) changed.push('role_id')
  const av0 = existing.avatarUrl ?? null
  const av1 = projected.avatarUrl ?? null
  if (av0 !== av1) changed.push('avatar_url')
...
export async function serviceListUsers(
  pool: pg.Pool,
...
export async function serviceCreateUser(
  ...
  if (!roleOk) {
    throw new AppError(
      'Papel de acesso não encontrado.',
      422,
      ErrorCodes.VALIDATION_ERROR,
    )
  }
  ...
  const input: InsertAppUserInput = {
    email: body.email,
  ...
  const newId = await withTransaction(pool, async (client) => {
    const id = await insertAppUser(client, input).catch((e: unknown) => {
      handlePg(e)
    })
...
export async function servicePatchUser(
  pool: pg.Pool,
  actorUserId: string,
  id: string,
  body: PatchAdminUserBody,
  ...
  if (body.collaboratorId !== undefined && body.collaboratorId !== null) {
    const ok = await assertCollaboratorEligibleForLink(
      pool,
      body.collaboratorId,
      id,
    )
  ...
  const patch: PatchAppUserInput = { updated_by: actorUserId }
  if (body.email !== undefined) patch.email = body.email
  if (body.roleId !== undefined) patch.role_id = body.roleId
  if (body.collaboratorId !== undefined) patch.collaborator_id = body.collaboratorId
  if (body.avatarUrl !== undefined) patch.avatar_url = emptyUrl(body.avatarUrl)
  ...
  try {
    await withTransaction(pool, async (client) => {
      ...
      if (body.password !== undefined) {
        const passwordHash = await hashPassword(body.password)
        const pwOk = await setAppUserPasswordExplicit(
          client,
          id,
          passwordHash,
          actorUserId,
...
export async function serviceInactivate(
  ...
  if (actorUserId === id) {
    throw new AppError(
      'Não pode inativar a sua própria conta.',
      422,
      ErrorCodes.VALIDATION_ERROR,
    )
  }
...
export async function serviceSoftDelete(
  pool: pg.Pool,
  actorUserId: string,
  id: string,
): Promise<AdminUserListItem> {
  ...
  await withTransaction(pool, async (client) => {
    ...
    if (!ok) {
      throw new AppError(
        'Não foi possível remover (já removido ou inexistente).',
        404,
        ErrorCodes.NOT_FOUND,
      )
    }
...
export async function serviceRestore(
  pool: pg.Pool,
  actorUserId: string,
  id: string,
): Promise<AdminUserListItem> {
  await withTransaction(pool, async (client) => {
    ...
    if (!ok) {
      throw new AppError(
        'Não foi possível restaurar (não está removido ou inexistente).',
        404,
        ErrorCodes.NOT_FOUND,
      )
    }
...
export async function serviceForcePasswordChange(
  pool: pg.Pool,
  actorUserId: string,
  id: string,
): Promise<AdminUserListItem> {
...
export async function serviceResetPassword(
  ...
  if (actorUserId === id) {
    throw new AppError(
      'Não pode redefinir a sua própria senha por aqui. Utilize «Alterar senha» na sua conta.',
      422,
      ErrorCodes.VALIDATION_ERROR,
    )
  }
  ...
  await withTransaction(pool, async (client) => {
    const ok = await resetAdminUserPasswordHash(
      client,
      id,
      passwordHash,
      actorUserId,
...
export async function serviceEligibleCollaborators(
  pool: pg.Pool,
