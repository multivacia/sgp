  ...
  .transform((data) => {
    ...
    const status =
      statusRaw === undefined || statusRaw === ''
        ? undefined
        : (statusRaw.toUpperCase() as 'ACTIVE' | 'INACTIVE' | 'ALL')
    const sectorRaw = data.sector_id?.trim()
    const sector_id =
      sectorRaw === undefined || sectorRaw === ''
        ? undefined
    ...
    const role_id =
      roleRaw === undefined || roleRaw === '' ? undefined : roleRaw
    const searchRaw = data.search?.trim()
    const search =
      searchRaw === undefined || searchRaw === '' ? undefined : searchRaw
    const linkedRaw = data.linked_user?.trim().toLowerCase()
    const linked_user: z.infer<typeof linkedScope> =
      linkedRaw === undefined || linkedRaw === ''
        ? 'all'
        : (linkedRaw as 'linked' | 'unlinked' | 'all')
    const deletedRaw = data.deleted?.trim().toLowerCase()
    const deleted: z.infer<typeof deletedScope> =
      deletedRaw === undefined || deletedRaw === ''
        ? 'exclude'
        : (deletedRaw as 'exclude' | 'only' | 'include')
    const limit = Math.min(
      500,
      Math.max(1, Number.parseInt(data.limit ?? '200', 10) || 200),
    )
...
export type ListAdminCollaboratorsQuery = z.infer<
  typeof listAdminCollaboratorsQuerySchema
>
