...
import type { PlanningActivityTicketPrintGroupingMode } from './activityTicketPlanningGrouping'

export type PlanningWeekTicketsPrintDialogOptions = {
  grouping: PlanningActivityTicketPrintGroupingMode
  includeCompleted: boolean
}

type PlanningWeekTicketsPrintDialogProps = {
  open: boolean
  ...
  onClose: () => void
  onPrint: (options: PlanningWeekTicketsPrintDialogOptions) => void
  onDismissAgentNotice?: () => void
  onTestThermalPrinter?: () => void
}

const GROUPING_OPTIONS: Array<{ value: PlanningActivityTicketPrintGroupingMode; label: string }> = [
  { value: 'responsible', label: 'Responsável' },
  { value: 'task', label: 'Tarefa / esteira' },
]

export function PlanningWeekTicketsPrintDialog(
  props: PlanningWeekTicketsPrintDialogProps,
) {
  const [grouping, setGrouping] = useState<PlanningActivityTicketPrintGroupingMode>('responsible')
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal
      aria-labelledby="planning-week-tickets-print-title"
    ...
    >
      ...
      >
        <h2
          id="planning-week-tickets-print-title"
        ...
        {emptyMessage ? (
          <p className="mt-4 text-[13px] text-slate-400">{emptyMessage}</p>
        ) : (
          <>
            <label className="mt-5 block text-[12px] text-slate-400">
              ...
              <select
                ...
                value={grouping}
                onChange={(e) =>
                  setGrouping(e.target.value as PlanningActivityTicketPrintGroupingMode)
                }
              >
            ...
            <p className="mt-3 text-[11px] text-slate-500">
              {includeCompleted
                ? `${props.totalCount} atividade(s) planejada(s) na semana.`
