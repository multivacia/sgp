import type { AppShellFunctionId } from '../../lib/shell/app-nav-config'

const LABEL: Record<AppShellFunctionId, string> = {
  gestao: 'Gestão',
...
export function FunctionSwitchConfirmDialog({ open, target, onConfirm, onCancel }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal
      aria-labelledby="shell-function-switch-title"
      onClick={onCancel}
    >
      <div
      ...
      >
        <h2
          id="shell-function-switch-title"
          className="font-heading text-lg font-bold tracking-tight text-slate-50"
        >
          Trocar de função?
        </h2>
        <p className="mt-3 text-sm leading-relaxed text-slate-400">
          Ao mudar para <span className="font-semibold text-slate-200">{LABEL[target]}</span>, o
          contexto atual desta página (filtros, seleções ou alterações ainda não guardadas) pode ser
          descartado.
        </p>
