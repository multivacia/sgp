...
| Data da revisão v3 | 2026-08-08 |
| Data da correção v3.1 (esta versão) | 2026-08-08 |
| **Branch** | Originalmente `docs/analise-responsavel-atividade-matriz` (PR #12); trazida para `feature/responsavel-atividade-matriz` na implementação da Etapa 1 |
...
| v3 | Correção após 2ª revisão do time | `SELECT_ALTERNATIVE` remapeado com investigação ponta a ponta (gap estrutural, distinto de `ACCEPT_SUGGESTED`); duplicação com responsável inválido redefinida como sucesso sem responsável **com aviso explícito** (nunca descarte silencioso); proteção de integridade Equipes/Colaboradores formalizada como Etapa 2; `draftToCreateConveyorInput.ts` classificado corretamente como **frontend** em todo o documento. |
| v3.1 (esta versão) | Três correções objetivas antes da implementação, sem reabrir revisão de impacto | (1) Estimativa ajustada para 5–7 dias úteis para um implementador; (2) afirmação categórica de ausência de dados legados trocada por pendência de confirmação em HML/PRD; (3) caso de teste de "selecionar novamente a mesma equipe" reformulado para não sugerir troca real de equipe mantendo responsável potencialmente inválido. |
...
| # | Caso | Resultado esperado |
|---|---|---|
| 14 | Criar/reutilizar opção de catálogo selecionando novamente a mesma equipe (sem troca real de equipe) | Responsável não é zerado indevidamente — **correção v3.1**: o caso testa reconfirmar a mesma equipe já selecionada, não uma troca real de equipe com responsável potencialmente incompatível (esse cenário já está coberto pelos casos 8 e 9) |
...
**Compatibilidade com dados existentes**: o fluxo normal atual não permite gravar responsáveis — confirmado por leitura de todo caminho de escrita do backend nas três revisões (criação, patch e duplicação sempre rejeitam ou zeram `default_responsible_id` para `ACTIVITY`). **Correção v3.1**: isso comprova que a via normal da aplicação não produz esse dado; não comprova, por si só, a ausência de qualquer valor gravado por caminho fora do fluxo normal (acesso direto ao banco, script, seed, versão anterior do código). A inexistência de dados legados ou inseridos diretamente ainda precisa ser confirmada em HML/PRD antes do deploy. Como o campo é opcional e nulável, mesmo que existam valores legados isolados, eles não quebram nenhum fluxo — não há dado a migrar ou reconciliar em nenhum dos dois cenários.
...
A estimativa da v2 (2–3 dias) ficou apertada após os achados de catálogo, atomicidade e, principalmente, o gap estrutural de `SELECT_ALTERNATIVE`. **Correção v3.1**: a decomposição por frente abaixo soma otimisticamente 5 a 8 pessoa-dias de trabalho especializado (backend de matriz, contratos R6, frontend R6, frontend de matriz, testes); tratar isso como "3 a 5 dias úteis" só seria correto assumindo paralelismo real entre pelo menos duas pessoas nas frentes de backend e frontend. Para um único implementador, sem paralelismo, a referência correta é **5 a 7 dias úteis**:
...
| **Total (1 implementador, sem paralelismo)** | **5 a 7 dias úteis** | Concentrado em backend (validação + atomicidade + contratos R6) e na correção coordenada backend→frontend do `SELECT_ALTERNATIVE`; a soma direta das frentes já ultrapassa 5 dias, com folga adicional para revisão/correção |
